...
fileprivate var buttonSize:Int = 16
/// A radio button with a fill animation.
@IBDesignable open class RadioButton: UIButton {
    @IBOutlet var container: UIButton!
    ...
    func initNib() {
        ...
        bundle.loadNibNamed("Radio", owner: self, options: nil)
    ...
    @IBInspectable open var radioColor: UIColor = UIColor.black{
        didSet{
            self.selectedColor = radioColor
        }
    }
    @IBInspectable open var radioSize: Int {
        set{
            buttonSize = newValue
            self.frame = CGRect(x: 0, y: 0, width: newValue, height: newValue)
        }
        get{
            return buttonSize
        }
    ...
    @objc private func toggleState() {
        guard isChecked else { return select() }
    ...
    public init(diameter: CGFloat = 16, selectedColor: UIColor? = nil, deselectedColor: UIColor? = nil) {
